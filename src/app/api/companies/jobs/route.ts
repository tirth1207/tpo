...
// ---------------------------
// GET: Fetch all jobs for company
// ---------------------------
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('jobs').select('*').eq('company_id', company.id)
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { error: updateError } = await supabase.from('jobs').update(updateFields).eq('id', jobId)
